...
protected:
  ...
  inline static CAccessibleType& get();
};
...
public:
  ...
  inline const TAccessible* operator->() const;
  inline TAccessible* operator->();
  inline const TAccessible& operator*() const;
  inline TAccessible& operator*();
};
...
public:
  ...
  inline CAnyGlobalInitializer(TArgs&& ... args);
};


template<class TAccessible, class TID>
inline typename CAnyGlobalAccessible<TAccessible, TID>::CAccessibleType&
CAnyGlobalAccessible<TAccessible, TID>::get() {
...
template<class TAccessible, class TID>
inline CAnyGlobalAccess<TAccessible, TID>::CAnyGlobalAccess() {
  assert(CBase::get());
}

template<class TAccessible, class TID>
inline const TAccessible* CAnyGlobalAccess<TAccessible, TID>::operator->() const {
  assert(CBase::get());
  return CBase::get().get();
}

template<class TAccessible, class TID>
inline TAccessible* CAnyGlobalAccess<TAccessible, TID>::operator->() {
  assert(CBase::get());
  return CBase::get().get();
}
...
template<class TAccessible, class TID>
template<class... TArgs>
inline CAnyGlobalInitializer<TAccessible, TID>::
